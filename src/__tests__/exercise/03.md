...
- Using a `screen` query from the React testing library most closely resembles
  the way a user would find a certain control. See priority of query types
  [here](https://testing-library.com/docs/queries/about/#types-of-queries)
- Using the `userEvent` utility from the React testing library most closely
  resembles the way a user would interact with a control
