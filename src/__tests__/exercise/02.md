...
- React testing library does not require us to supply our own div when rendering
  a component
- React testing library handles a lot of clean up for us
- React testing library has a `fireEvent` utility that cleans up a lot of event
  boiler plate
- React testing library also has some useful expect extensions. To use them
  `@testing-library/jest-dom` must be imported into the project at some point
