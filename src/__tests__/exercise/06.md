...
- We can mock browser APIs that third party modules may make use of. Then our
  tests will still use the third party module, but use our mock implementation
  for a specific API
- It can be a lot of work to mock a specific API like that, so if we aren't too
  worried about it we can mock the third party module instead. This removes some
  confidence we may have in our tests, but with a third party module we can
  likely assume that as long as we call it correctly it should work
