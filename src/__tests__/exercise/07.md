...
- In order to test a component that uses context, we must render the component
  along with the Content Provider
- To accomplish this the React Testing Library `render` method accepts a
  `wrapper` option. The wrapper should return the Context Provider with the
  component under test rendered as a child
- To handle Content Providers cleanly we can create a custom render method that
  will render a component along with all of our app's Context Providers. It's
  best to do this in a test util file that will import everything from react
  testing library and re-export it, overriding the render method with our custom
  render method. See this exercise for an example
