...
- We can mock the backend in tests using `rest` and `setupServer` utilities from
  `msw` (see exercise)
- We also can use the `waitForElementToBeRemoved` to ensure we wait until
  loading is done to assert the result
- To reuse server handlers, we can store them in a separate file, import them,
  and send them along directly to `setupServer`
- `toMatchInlineSnapshot` is a useful test utility that will inline a snapshot
  of the element that is retrieved from the screen. If the test result is
  different from the snapshot the test will fail and we can update the snapshot
  by typing in `u`
- To use a one-off server handler we can use `server.use` to add handlers to the
  already-running server. To reset the handlers after each test add an
  `afterEach` that calls `server.resetHandlers()`
