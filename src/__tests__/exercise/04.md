...
- When testing a form that accepts an onSubmit function, it can be useful to
  send a jest mock function and assert what that function is called with and how
  many times it it called
- When sending mock data it can be useful to use generated data to show that the
  exact values sent in aren't important. The `faker` library can be used to
  accomplish this
- When generating data it can be useful to use the `@jackfranklin/test-data-bot`
  module that will use a factory to create the test data so it's not regenerated
  unneccessarily
